...
export class App extends Component {
  ...
  handleSubmit = (e) => {
    e.preventDefault();
    axios.get(`${BASE_URL}news?access_key=${API_KEY}&keywords=${this.state.search_input}&countries=us`)
      .then(res => {
  ...
  handleClickOnSport = async () => {
    const res = await axios.get(`${BASE_URL}news?access_key=${API_KEY}&categories=sports&countries=us`);
    this.setState({ searchArray: res.data.data })
  }

  handleClickOnTech = async () => {
    const res = await axios.get(`${BASE_URL}news?access_key=${API_KEY}&categories=technology&countries=us`);
    this.setState({ searchArray: res.data.data })
  }
  ...
  render() {
    return (
      <div className='app-container'>
        ...
        <NewsList news={this.state.searchArray} />
      </div>
    )
  }
}
